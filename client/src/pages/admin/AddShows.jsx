...
const AddShows = () => {
  ...
  const fetchAddedShows = async () => {
    try {
      const { data } = await axios.get('/api/show/all', {
        headers: { Authorization: `Bearer ${await getToken()}` }
      });
      if (data.success) {
        setAddedShows(data.shows);
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (data.success) {
        ...
        setShowPrice("")
        fetchAddedShows(); // Refresh the shows list
      } else {
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      <div className="mt-6">
        ...
        <div className="inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg">
          ...
          <button
            ...
            className="bg-primary/80 text-white px-2 py-2 text-sm rounded-lg hover:bg-primary cursor-pointer"
          >
            Add Time
          </button>
        </div>
      ...
      {addedShows.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-semibold mb-4">Added Shows</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {addedShows.map((show) => (
              <div
                key={show._id}
                className="border border-primary/20 rounded-lg p-3 shadow-md bg-primary/5 relative"
              >
                <button
