...
import { MenuIcon, SearchIcon, TicketPlus, XIcon, Settings } from 'lucide-react'
...
const Navbar = () => {
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-32 py-5'>
     ...
     <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-1g z-50 flex flex-col md:flex-row items-center max-md:justify-center gap-8 min-md:px-8 py-3 max-md:h-screen min-md: rounded-full backdrop-blur bg-black/70 md:bg-white/10 md:border border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-full' : 'max-md:w-0'}`}>
        ...
        {isAdmin && (
          <Link onClick={()=> {scrollTo(0,0); setIsOpen(false)}} to='/admin' className='flex items-center gap-1 text-primary'>
     ...
     {isOpen && (
        <XIcon
          className='md:hidden fixed top-15 right-20 w-6 h-7 text-white z-[999] cursor-pointer'
